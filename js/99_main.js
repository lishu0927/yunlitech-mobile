 ...
 */
 	...
 	page_translate	: function(node){
		...
		y_2 = y_2/5;
		node[1].style[car2._prefixStyle('transform')] = 'translate(0,0)'+_translateZ;
 	},
 	...
 	page_success	: function(){
		...
		//car2._page.eq(car2._pageNow)[0].style[car2._prefixStyle('transform')] = 'translate(0,'+y+'px)'+_translateZ
 		// 成功事件
 ...
 */
	...
	lazy_img : function(){
		...
		lazyNode.each(function(){
			var self = $(this);
			if(self.is('img')){
				self.attr('src','../images/loading.gif');
			}else{
				// 把原来的图片预先保存下来
 ...
 */
	...
	refresh	: function(){
		...
		console.log(car2._windowHeight);
		console.log(car2._windowWidth)
	},
...
function testclassname(){
	var _page2=$('.page-con-2');
	var classnames=_page2.parent().attr('class');
	console.log(classnames);
	//console.log(typeof(classnames));
	var pageshow=/m-page active/;
	var pageshows=/m-page z-animate active/;
	//console.log(pageshow.test(classnames));
	if(pageshow.test(classnames)||pageshows.test(classnames)){
		setTimeout(function(){
			$('.timearis .title-img').css('width','38px')
			$('.timearis .title-img').css('opacity','1');

		},500);
		//setTimeout(function(){
		//	$('.timearis ul li .title-line').css('opacity','1');
		//	$('.timearis ul li .title-line').css('height','100%');
		//	$('.timearis .timearis-ani').css('height','100%');
		//},1500);

	}else {
		//$('.timearis .timearis-ani').css('height','50px');
		//$('.timearis ul li .title-line').css('opacity','0');
		//$('.timearis ul li .title-line').css('height','0');
		$(' .timearis .title-img').css('width','50px');
		$(' .timearis .title-img').css('opacity','0');
	}
}
